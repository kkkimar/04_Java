...
public class ExceptionHandlingService1 {
	 ...
	 * 1) throw : 예외 강제 발생(던짐)
	 * 2) 예외 + 다형성
	 * 	- catch문 여러개를 작성시 부모 예외를 처리하는 catch문이 자식 예외도 모두 잡아서 처리
	...
	public void method5() {
		...
		do {
			...
			// 자식 catch문까지 코드가 도달할 수 없다! (수행불가, 죽은 코드)
			
			// 해결 방법: 부모 catch를 자식 catch 밑으로 이동
			...
			// Unreachable catch block : 도달할 수 없는 catch구문
			catch(InputMismatchException e) {
