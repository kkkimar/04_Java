...
public class ExceptionHandlingService3 {
	 ...
	 * 	 -> 예외 처리 구문(try-catch , throws)을 반드시! 작성해야하는 예외
	 * 
	 * - UnChecked Exception : 확인 안 해도 되는 예외
	 * 	 -> 예외 처리 구문을 써도 그만, 안써도 그만인 예외
	 * 
	 * 
	 * 
	 * */
	...
	public void method1() {
		...
		//RuntimeException, RuntimeException 자식 제외한
		//나머지 예외 강제 발생 
			...
			//-> IOException 예외 처리가 안되어 있음  => 예외처리 꼭 해라 
	...
	public void method2() {
		...
		//		integer.parsInt(문자열), Double.parseDouble(문자열) 이용
		...
		// -> IOException 예외 처리 구문이 필수적으로 작성 되어야 한다 !
	...
	public void method3() {
		...
		try {
			...
			//-> 파싱하려는 문자열 input이 숫자의 형태가 아니라서 예외가 발생
	...
	public void method4() {
		...
		//-> 호출한 곳에서 처리하겠다 -> 예외처리 코드(try-catch, throws) 작성
		
		System.out.print("두번째 문자열 입력 : ");
		String str2 = inputString();
		
//		System.out.println(str1 + str2); -> 효율 별로임 !
	...
	public String inputString() throws IOException {
		...
		// throws IOException :
		// 현재 메서드에서 발생할 수 있는(던져질 수 있는) throws IOException을 
		// 현재 메서드를 호출한 곳으로 던져서  
		// 상황에 맞게 별도로 처리하게 함
	...
	public void method5() {
		...
		try {
			...
			System.out.println("[두정수를 입력 받아 합계 출력하기]");
			...
			if(result > 100) { //100초과한 경우 심각한 상황이라 가정
				
//				throw new MoreThanAHundredException();
				throw new MoreThanAHundredException("100 초과아아아아아");
			...
			// e.printStackTrace() 
			// - print : 출력하다
			// - stack : JVM 메모리 중, Stack여역(메서드가 쌓여있음)
			// - Trace : 추적하다
			
			//-> 호출한 메서드 순서대로 발생한 예외를 모두 추적
			e.printStackTrace();
		}
