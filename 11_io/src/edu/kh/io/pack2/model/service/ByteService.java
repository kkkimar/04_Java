...
public class ByteService {
	...
	// -> IOException 예외 처리 구문 작성하는 것이 필수 ! 
	...
	public void fileByteOutput() {
		...
		FileOutputStream fos = null; // try, finally 구문에서 사용 예정 -> 미리 밖에 선언
		...
		try {
			...
			//    1byte 기반 스트림 통과 시 데이터 손실이 발생해서 글자가 깨지는 문제가 발생!
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * * 버퍼링
		 * - 인터넷 속도 낮음 + 스트리밍 코드의 효율이 안좋음
		 * 	 -> 동영상 재생 중에 미리 앞에 재상될 부분을 다운받아 놔서
		 * 			영상을 끊김 없이 볼 수 있게 함
		 * 			-> 다운 받아서 저장 해 놓은 공간 == 버퍼
		 * 
		 * */
		
		
		/*
		 * BufferedOutputStream / BufferedInputStream -> 실무에서도 많이 씀 ! 필수!
		 * 
		 ...
		 * 
		 * 	=> 스트림을 이용하는 횟수가 적어짐 -> 성능과 시간 효율 UP!
		 * 	
		 * */
		
		
		FileOutputStream fos = null; // try, finally에서 사용예정 
		...
		try {
			...
			try {
				...
				if(bos != null) {
					bos.close();
					// 보조 스트림 close() 시 보조 스트림 생성에 사용된 기반 스트림도 같이 close() 됨
...
//			fis.flush() -> InputStream은 flush() 없음
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			// 파일 내욜을 byte[] 배열로 반환 받아 String으로 변경
	...
	public void fileCopy() {
		...
		try {
			...
			fis = new FileInputStream(target); //기반스트림
			...
			StringBuilder sb = new StringBuilder();
			sb.append(target); //io_test/20240215/파일생성.txt
